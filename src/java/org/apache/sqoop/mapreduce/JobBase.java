...
/**
 * Base class for configuring and running a MapReduce job. Allows dependency
 * injection, etc, for easy customization of import job types.
 */
public class JobBase {
	...
	protected void cacheJars(Job job, ConnManager mgr) throws IOException {
		...
		addToCache(job.getConfiguration().get("mapred.jar"), fs, localUrls);
		...
		if (null != tool) {
			// Make sure the jar for the tool itself is on the classpath. (In
			// case
	...
	protected void loadJars(Configuration conf, String ormJarFile,
		...
		boolean isLocal = "local".equals(conf
				.get("mapreduce.jobtracker.address"))
				|| "local".equals(conf.get("mapred.job.tracker"));
		if (isLocal) {
			// If we're using the LocalJobRunner, then instead of using the
			// compiled
			// jar file as the job source, we're running in the current thread.
			// Push
	...
	/**
	 * Configure the number of map/reduce tasks to use in the job, returning the
	 * number of map tasks for backward compatibility.
	 */
	...
	protected int configureNumReduceTasks(Job job) throws IOException {
		...
		if(options.isUseReducePhaseForPartitioning()){
			numReduceTasks = options.getNumReducers();
			
			if (numReduceTasks < 1) {
				numReduceTasks = SqoopOptions.DEFAULT_NUM_REDUCERS;
				LOG.warn("Invalid reducer count; using " + numReduceTasks + " mappers.");
			}
		}else{
			numReduceTasks = 0;
		}
		
		ConfigurationHelper.setJobNumReduce(job, numReduceTasks);
	...
	/**
	 * @return the main MapReduce job that is being run, or null if no job has
	 *         started.
	 */
	public Job getJob() {
		return mrJob;
	 ...
	 *
	 * @param configuration
	 *            Hadoop configuration that should be used
	 ...
	 *
	 * @param opts
	 *            SqoopOptions that should be serialized
	 * @param configuration
	...
	public void putSqoopOptionsToConfiguration(SqoopOptions opts,
		...
		for (Map.Entry<Object, Object> e : opts.writeProperties().entrySet()) {
			...
			// We don't need to do if(value is empty) because that is already
			// done
	 ...
	 */
	protected boolean runJob(Job job) throws ClassNotFoundException,
			IOException, InterruptedException {
		return job.waitForCompletion(true);
	}

	/**
	 * Display a notice on the log that the current MapReduce job has been
	 * retired, and thus Counters are unavailable.
	 * 
	 * @param log
	 *            the Log to display the info to.
	 */
	 ...
	 * some of them to the job itself, so that they can be easily accessed.
	 * We're propagating only interesting global options (like verbose flag).
	 *
	 * @param job
