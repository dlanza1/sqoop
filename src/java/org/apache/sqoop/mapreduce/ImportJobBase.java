...
public class ImportJobBase extends JobBase {
	...
	public static final Log LOG = LogFactory.getLog(ImportJobBase.class
			.getName());
	...
	protected void configureOutputFormat(Job job, String tableName,
		...
		if (isHCatJob) {
			...
			SqoopHCatUtilities.configureImportOutputFormat(options, job,
					getContext().getConnManager(), tableName,
					job.getConfiguration());
			return;
		}

		if (options.getFileLayout() == SqoopOptions.FileLayout.SequenceFile) {
			job.getConfiguration().set("mapred.output.value.class",
					tableClassName);
		}
	...
	protected boolean runJob(Job job) throws ClassNotFoundException,
		...
		if (isHCatJob) {
			SqoopHCatUtilities.instance()
					.invokeOutputCommitterForLocalMode(job);
		}
	 ...
	 */
	protected boolean doSubmitJob(Job job) throws IOException,
	 ...
	 */
	public void runImport(String tableName, String ormJarFile,
			String splitByCol, Configuration conf) throws IOException,
			ImportException {
		...
		if (options.getAccumuloTable() != null
				&& options.isDirect()
				&& !getContext().getConnManager()
						.isDirectModeAccumuloSupported()) {
		...
		try {
			...
			configureReducer(job, tableName, tableClassName);
	...
	private Class<SqoopRecord> getRecordClass(String jar, String tableClassName)
			throws IOException {

		// Get class loader for the jar file
		...
		try {
			// return class
			return (Class<SqoopRecord>) Class.forName(tableClassName, true,
					classLoader);
