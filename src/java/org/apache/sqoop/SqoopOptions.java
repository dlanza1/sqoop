...
public class SqoopOptions implements Cloneable {
	...
	public static final Log LOG = LogFactory.getLog(SqoopOptions.class
			.getName());
	...
	@StoredAsProperty("mapreduce.num.reducers")
	...
	public static final int DEFAULT_NUM_REDUCERS = 4;
	...
	// The import should be partitioned
	@StoredAsProperty("import.useReducePhaseForPartitioning")
	private boolean useReducePhaseForPartitioning;
	...
	private boolean getBooleanProperty(Properties props, String propName,
		...
		String str = props
				.getProperty(propName, Boolean.toString(defaultValue))
				.toLowerCase();
	...
	private long getLongProperty(Properties props, String propName,
			long defaultValue) {
		String str = props.getProperty(propName, Long.toString(defaultValue))
				.toLowerCase();
		try {
	...
	private DelimiterSet getDelimiterProperties(Properties props,
		...
		boolean required = getBooleanProperty(props, prefix
				+ ".enclose.required", defaults.isEncloseRequired());
	...
	public void loadProperties(Properties props) {
		...
		try {
			...
			for (Field f : fields) {
				if (f.isAnnotationPresent(StoredAsProperty.class)) {
					Class typ = f.getType();
					StoredAsProperty storedAs = f
							.getAnnotation(StoredAsProperty.class);
					...
					if (typ.equals(int.class)) {
						...
						f.setBoolean(
								this,
								getBooleanProperty(props, propName,
										f.getBoolean(this)));
					} else if (typ.equals(long.class)) {
						f.setLong(
								this,
								getLongProperty(props, propName,
										f.getLong(this)));
						...
						String value = props.getProperty(propName,
								f.get(this) == null ? "null" : f.get(this)
										.toString());
						f.set(this, value.equals("null") ? null : new Integer(
								value));
					} else if (typ.isEnum()) {
						f.set(this, Enum.valueOf(typ, props.getProperty(
								propName, f.get(this).toString())));
					} else {
						throw new RuntimeException(
								"Could not retrieve property " + propName
										+ " for type: " + typ);
					}
				}
			}
			...
			throw new RuntimeException(
					"Illegal access to field in property setter", iae);
		}
		...
		if (this.jarDirIsAuto) {
			...
			this.jarOutputDir = getNonceJarDir(tmpDir + "sqoop-"
					+ localUsername + "/compile");
		}
		...
		this.connectionParams = getPropertiesAsNetstedProperties(props,
				"db.connect.params");

		// Loading user mapping
		this.mapColumnHive = getPropertiesAsNetstedProperties(props,
				"map.column.hive");
		this.mapColumnJava = getPropertiesAsNetstedProperties(props,
				"map.column.java");
	...
	private void loadPasswordProperty(Properties props) {
		...
		if (passwordFilePath != null) {
			try {
				password = CredentialsUtil.fetchPasswordFromLoader(
						passwordFilePath, getConf());
				return; // short-circuit
			} catch (IOException e) {
				throw new RuntimeException(
						"Unable to fetch password from file.", e);
			}
		}
				...
				if (f.isAnnotationPresent(StoredAsProperty.class)) {
					...
					if (typ.equals(int.class)) {
						putProperty(props, propName,
								Integer.toString(f.getInt(this)));
					} else if (typ.equals(boolean.class)) {
						putProperty(props, propName,
								Boolean.toString(f.getBoolean(this)));
					} else if (typ.equals(long.class)) {
						putProperty(props, propName,
								Long.toString(f.getLong(this)));
					} else if (typ.equals(String.class)) {
	...
	public Object clone() {
		try {
			...
			if (null != dbOutColumns) {
				other.dbOutColumns = Arrays.copyOf(dbOutColumns,
						dbOutColumns.length);
			}
			...
			if (null != outputDelimiters) {
				other.outputDelimiters = (DelimiterSet) outputDelimiters
						.clone();
			}
	...
	private void initDefaults(Configuration baseConfiguration) {
		...
		this.inputDelimiters = new DelimiterSet(DelimiterSet.NULL_CHAR,
		...
		this.numReducers = DEFAULT_NUM_REDUCERS;
	...
	public static char toChar(String charish) throws InvalidOptionsException {
		...
		if (charish.startsWith("\\0x") || charish.startsWith("\\0X")) {
			...
			if (charish.length() > 1) {
				LOG.warn("Character argument " + charish
	...
	/**
	 * @return the number of reduce tasks to use for import.
	 */
	...
	public boolean isUseReducePhaseForPartitioning() {
		return this.useReducePhaseForPartitioning;
	}

	public void setUseReducePhaseForPartitioning(boolean partitioned) {
		this.useReducePhaseForPartitioning = partitioned;
	}
}
